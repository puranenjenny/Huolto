...
if(isset($_GET['tehtava_id'])){
    ...
    $query = "SELECT tehtavat.tehtava_id, tehtavat.kuvaus, taloyhtiot.osoite, tilat.nimi, tehtavat.yleisavaimen_kaytto, tehtavat.numero, tehtavat.tehtavan_tilanne_id, tehtavan_tilanne.tehtavan_tilanne, tehtavat.jatetty, CONCAT(u.etunimi, ' ', u.sukunimi) AS ilmoittaja
    FROM tehtavat 
    ...
    INNER JOIN tilat ON tilat.tila_id  = tehtavat.tila_id            
    ...
    $JSON_paivita.= '{"ID":"'.$row['tehtava_id'].'","Viankuvaus":"'.$row['kuvaus'].'","Osoite":"'.$row['osoite'].'","Tila":"'.$row['nimi'].'","Yleisavain":"'.$row['yleisavaimen_kaytto'].'","Puhelin":"'.$row['numero'].'","TilaID":"'.$row['tehtavan_tilanne_id'].'","Tilanne":"'.$row['tehtavan_tilanne'].'","Paivays":"'.$row['jatetty'].'","Ilmoittaja":"'.$row['ilmoittaja'].'"}';
